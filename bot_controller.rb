...
class BotController < Telegram::Bot::UpdatesController
  def message(message)
    if message['text']
      ...
      respond_with :message, text: "ship" if text.include?('please go to @worrkinchat for offtopic ships')
      respond_with :message, text: "meetup" if text.include?('these violent delights have violent ends')
      respond_with :message, text: "MRR" if text.include?('MRR = Meme Reward Revenue')
